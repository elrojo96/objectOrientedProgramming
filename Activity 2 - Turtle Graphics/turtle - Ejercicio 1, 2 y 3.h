/*
    ...
    turtle.h para los ejercicios 1, 2 y 3
...
public:
    ...
    int move()  //Takes a line from the input file and executes the command in it
    {
        ...
        //Variable que toma toda la linea del archivo de texto en la que esta
        stringstream ssLinea;

        //Toma toda la linea del archivo de texto en la que esta
        getline(inFile, comment);
        ssLinea << comment;

        //Se asigna la primera palabra de la linea a la variable comment
        ssLinea >> comment;

        //Se compara para obtener el valor de command ya sea la instruccion string o numero
        if(comment == "PENUP" || comment == "1"){
            command = 1;
        }else if(comment == "PENDOWN" || comment == "2"){
            command = 2;
        }else if(comment == "RIGHT" || comment == "3"){
            command = 3;
        }else if(comment == "LEFT" || comment == "4"){
            command = 4;
        }else if(comment == "FORDWARD" || comment == "5"){
            command = 5;
        }else if(comment == "END" || comment == "9"){
            command = 9;
        }
